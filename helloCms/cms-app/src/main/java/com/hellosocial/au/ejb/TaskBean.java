...
public class TaskBean implements TaskLocal {
	...
	public void approveTask(String actorId, long taskId) throws Exception {
		...
		try {
			System.out.println("TaskDone (taskId = " + taskId + ") by "
					+ actorId);
	...
	@Override
	public List<TaskSummary> retrieveClaimedTaskList(String actorId)
			throws Exception {
		...
		try {
			list = taskService.getTasksOwned(actorId, "en-UK");
			ut.commit();
		} catch (RollbackException e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		...
		System.out.println("retrieveClaimedTaskList by " + actorId);
		for (TaskSummary task : list) {
			System.out.println(" task.getId() = " + task.getId());
		}
