...
import com.hellosocial.au.dao.ContentCatogoryDao;
import com.hellosocial.au.dao.UserDao;
import com.hellosocial.au.ejb.ContentLocal;
import com.hellosocial.au.ejb.TaskLocal;
import com.hellosocial.au.entity.Address;
import com.hellosocial.au.entity.Client;
import com.hellosocial.au.entity.Comment;
import com.hellosocial.au.entity.ContentCatogory;
import com.hellosocial.au.entity.Role;
import com.hellosocial.au.entity.Rule;
import com.hellosocial.au.entity.User;
import com.hellosocial.au.util.ExttraUtlis;
import com.hellosocial.au.util.ProcessEngineService;
...
public class ContentServiceImp implements ContentService {
	...
	public List<ContentCatogory> getContentCatogories(int clientId) {
		List<ContentCatogory> ContentCatogories = contentCatogoryDao
				.getContentCatogories(clientDao.getClient(clientId));
		return ContentCatogories;
	}

	@Override
	public List<ContentCatogory> getContentCatogoriesFromProcess(int processId) {
		int clientId = ((Client) processService.getProcessVariable(processId,
				"client")).getClientId();
	...
	public void createContent(String userId, String taskId,
		...
		ContentCatogory contentCatogory = contentCatogoryDao
				.getContentCatogory(Integer.parseInt((String) params
						.get("contentCatogoryId")));
		...
		params.put("hasGraphics_",
				Boolean.parseBoolean((String) params.get("hasGraphics")));
	...
	@Override
	public ContentCatogory getContentCatogoryFromTaskProcess(String processId) {
	...
	public ContentCatogory getContentCatogoryFromTaskProcess(int processId) {
		return (ContentCatogory) taskService.getTaskVariable(processId).get(
				"_contentCatogory");
	}
	...
	public Map<Integer, Comment> getComments(int taskId) {
		return (Map<Integer, Comment>) taskService.getTaskVariable(taskId).get(
				"_comments");
	}
	...
	public List<JSONObject> getRuleListJson(int contentCatogoryId)
			throws JSONException {
		List<JSONObject> jobj = new ArrayList<JSONObject>();
		Iterator<Rule> iterator = contentCatogoryDao.getRuleList(
				contentCatogoryId).iterator();
	...
	public Comment addComment(int processId, String comment, String userId) {
		// TODO Auto-generated method stub
		TreeMap<Integer, Comment> comments = (TreeMap<Integer, Comment>) processService
				.getProcessVariable(processId, "comments");
		User user = new User();
		user.setName(userId);
		...
		return ((Map<Integer, Comment>) processService.getProcessVariable(
				processId, "comments")).get(_comment.getCommentId());
	...
	public Map<Integer, Comment> getProcessComments(String processId) {
		return ((Map<Integer, Comment>) processService.getProcessVariable(
				Integer.parseInt(processId), "comments"));
	}
